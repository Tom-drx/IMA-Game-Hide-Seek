...
public class Hybrid8Test : MonoBehaviour
{
    ...
    private double fogDensity = 0.3; //initialise le fog a 0.3 de base 
    private int k = 0;
    private int pic = 570; //Le pic de l'utilisateur au repos A CHANGER POUR CHAQUE USER TEST!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    private int pickLast10Sec = 12; //le nombre de pic en 10 sec de l'utilisateur, il faudra initialiser la valeur selon chaque personne A CHANGER POUR CHAQUE USER TEST!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    private int nombrePick = 0; //Le nombre de pic dans l'interval de 10 sec actuel
    public AudioSource audioSource;//Audio Source
    private float volumeIncrement = 0.1f;//Valeur de l'increment du son
    private float volumeSon = 0.3f;//initialise le son a 0.3 de base
    private int compteurIntervallePic = 15; //c'est pour que 2 à 14 pics qui s'enchainent ne soient pas compter comme 2 à 14, mais comme 1 seul pic
    ...
    private void Update() //Update le fog et le son selon l'adaptabilité qui se trouve dans la fonction OnReceivedData 
    {
        RenderSettings.fog = true; 
        RenderSettings.fogColor = Color.gray; 
        RenderSettings.fogDensity = (float)fogDensity; 
...
public void OnDataReceived(int nSeq, int[] data)
{
    ...
    if (nSeq % samplingRate == 0)
    {
        ...
        for (int j = 0; j < data.Length; j++)
        {
            if(data[j] > 10) { //Pour enlever les valeurs entre 0 et 8 qui sont affichés par rapport au channel et qui nous intéresse pas.
                CreateText("\n" + data[j]);//enregistre la donnée dans un fichier texte
            }
            ...
            if (k < 11)
        {
                if (pic < data[j])
                {
                    ...
                    if (compteurIntervallePic < 1) //Pour que si plusieurs pics s'enchainent, seulement 1 soit pris en compte
                    {
                        CreateText("pic"); //Ajouter dans le text que cette valeur est un pic
                        nombrePick += 1;
                        compteurIntervallePic = 15;
                    }
                }
                else { 
                    compteurIntervallePic -= 1;
                }
                
        }
        else
        { //Selon le nombre de pic des 10 dernières secondes, adapter l'environnement et ajouter un récap pour avoir un trace dans le fichier texte.
            CreateText("\nLast 10 sec number of pic : "+ nombrePick + " Number of pic of precedent 10 sec : "+ pickLast10Sec + "\n");
            k = 0;
            if (nombrePick > pickLast10Sec)
            {
                fogDensity = fogDensity - 0.1;
                volumeSon = Mathf.Clamp(audioSource.volume + volumeIncrement, 0f, 1f);
            }
            else if (nombrePick < pickLast10Sec)
            {
                fogDensity = fogDensity + 0.1;
                volumeSon = Mathf.Clamp(audioSource.volume - volumeIncrement, 0f, 1f);
            }
            pickLast10Sec = nombrePick;
            nombrePick = 0;
        }
        }
    }
    //Pour stocker nos données dans un fichier texte pour ensuite les analyser.
    public void CreateText (string data) { 

        // Create a file path using the desired folder and file name
        string path = Application.dataPath + "/Test.txt"; // A CHANGER POUR CHAQUE USER TEST!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
